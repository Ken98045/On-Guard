...
namespace SAAI
{
  ...
  [Serializable]
  public class AllCameras : IDisposable
  {
    ...
    public AllCameras(AllCameras src)
    {
      CameraDictionary = new Dictionary<string, CameraData>();
    ...
    /// </summary>
    /// <returns></returns>
    public static AllCameras Load()
    {

      AllCameras all = new AllCameras();

      if (File.Exists("CameraData.bin"))
      {
        BinaryFormatter serializer = new BinaryFormatter();
        using (Stream reader = new FileStream("CameraData.bin", FileMode.Open))
        {
          all = (AllCameras)serializer.Deserialize(reader);
        }
      }
    ...
    public static void Save(AllCameras allCameras)
    {
      BinaryFormatter serializer = new BinaryFormatter();
      using (Stream stream = new FileStream("CameraData.bin", FileMode.Create))
      {
